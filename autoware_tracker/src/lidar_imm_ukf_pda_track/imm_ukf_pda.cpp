...
// private_nh_("~")
{
        ...
        private_nh_.param<std::string>("autoware_tracker/tracker/kitti_data_dir", kitti_data_dir_, "/home/tianbot/online_learning_ws/src/autoware_tracker/benchmark/");
        if (is_benchmark_)
        {
...
void ImmUkfPda::callback(const autoware_tracker::DetectedObjectArray &input)
{
        ...
        autoware_tracker::DetectedObjectArray input_copy;
        input_copy = input;
        ...
        // ROS_INFO("multi-object tracker: %lf s", current_time - start_time);
        std::cout << "multi-object tracker: " << current_time - start_time << "s" << std::endl;

        start_time = ros::WallTime::now().toSec();
        if (is_benchmark_)
        {
                dumpResultText(detected_objects_output);
                // dumpResultText(input_copy);
        }
        ...
        for (size_t i = 0; i < detected_objects_output.objects.size(); i++)
        {
                ...
                if (detected_objects_output.objects[i].label == "1" && detected_objects_output.objects[i].score < 0.75)
                {
                        continue;
                }
                if (detected_objects_output.objects[i].label == "0" && detected_objects_output.objects[i].score < 0.75)
                {
                        continue;
                }
                if (detected_objects_output.objects[i].label == "2" && detected_objects_output.objects[i].score < 0.5)
                {
                        continue;
                }
        ...
        for (size_t i = 0; i < learning_buffer.size(); i++)
        {
                if (learning_buffer[i].objects.back().last_sample || learning_buffer[i].objects.size() >= 5)
                {
                        std::map<std::string, double> label_probabilities;

                        for (size_t j = 0; j < learning_buffer[i].objects.size(); j++)
                        {
                                if (learning_buffer[i].objects[j].label.compare("9") != 0)
                                {
                                        ...
                                        for (size_t k = 0; k < label_probabilities.size(); k++)
                                        {
                                                if (label_probabilities.find(learning_buffer[i].objects[j].label) != label_probabilities.end())
                                                {
                                                        ...
                                                        {
                                                                label_probabilities[learning_buffer[i].objects[j].label] *= 10;
                                                                
                                                        }
                                                        else
                                                        {
                                                                label_probabilities[learning_buffer[i].objects[j].label] *= learning_buffer[i].objects[j].score / (1 - learning_buffer[i].objects[j].score);
                                        ...
                                        if (new_label)
                                        {
                                                if (learning_buffer[i].objects[j].score >= 1.0)
                                                {
                                                        label_probabilities.insert(std::pair<std::string, double>(learning_buffer[i].objects[j].label, 10));
                                                }
                                                else
                                                {
                                                        label_probabilities.insert(std::pair<std::string, double>(learning_buffer[i].objects[j].label, learning_buffer[i].objects[j].score / (1 - learning_buffer[i].objects[j].score)));
                                                }
                                        }
                                }
                        }
                        ...
                        if (product_odds_max / (1 + product_odds_max) >= track_probability_ && learning_buffer[i].objects.size() > 1)
                        {
                                ...
                                if (learning_buffer[i].objects[0].label.compare("9") != 0)
                                {
                                        pub_example_array_.publish(learning_buffer[i]);
                                }
                                if (learning_buffer[i].objects[0].label.compare("9") != 0)
                                ...
                                for (size_t k = 0; k < detected_objects_output.objects.size(); k++)
                                {
                                        pcl::concatenatePointCloud(pc, detected_objects_output.objects[k].pointcloud, pc);
                                }
        ...
        current_time = ros::WallTime::now().toSec();
        // ROS_INFO("Sample Annotator : %lf s", current_time - start_time);

        std::cout << "Sample Annotator : " << current_time - start_time << "s" << std::endl;
        // pub_object_array_.publish(detected_objects_output);
        // yang21itsc
}
...
{
        target.lifetime_++;
        if (!target.object_.label.empty() && target.object_.label != "9")
        {
        ...
        for (size_t i = 0; i < matching_objects.size(); i++)
        {
                ...
                for (size_t j = 0; j < matching_objects[i].size(); j++)
                {
                        ...
                        if (!targets_[in_tracker_indices[current_index]].label_.empty() &&
                            targets_[in_tracker_indices[current_index]].label_ != "9")
                        {
                ...
                if (best_label != "9" && !best_label.empty())
                {
...
void ImmUkfPda::dumpResultText(autoware_tracker::DetectedObjectArray& detected_objects)
{
        ...
        for (size_t i = 0; i < detected_objects.objects.size(); i++)
        {
                ...
                double alpha = atan((detected_objects.objects[i].pose.position.x)/(-detected_objects.objects[i].pose.position.y));
                double yaw = tf::getYaw(detected_objects.objects[i].pose.orientation);
                std::cout << " imm ukf pda " << detected_objects.objects[i].pose<<"   " << detected_objects.objects[i].yaw << "  " << yaw  << std::endl;
...
//yao
                if (detected_objects.objects[i].label == "0")
                {
                        detected_objects.objects[i].label = "Car";
                }
                else if (detected_objects.objects[i].label == "1")
                {
                        detected_objects.objects[i].label = "Pedestrian";
                }
                else if (detected_objects.objects[i].label == "2")
                {
                        detected_objects.objects[i].label = "Cyclist";
                }
                else{
                        detected_objects.objects[i].label = "9";
                }
                outputfile << std::to_string(frame_count_) << " " << std::to_string(detected_objects.objects[i].id) << " "
                // outputfile << detected_objects.objects[i].label
                           << detected_objects.objects[i].label
                           << " "
                           << "-1"
                           << " "
                           << "-1"
                           << " "
                           << std::to_string(alpha) << " "
                           << std::to_string(detected_objects.objects[i].x) << " "
                           << std::to_string(detected_objects.objects[i].y) << " "
                           << std::to_string(detected_objects.objects[i].x + detected_objects.objects[i].width) << " "
                           << std::to_string(detected_objects.objects[i].y + detected_objects.objects[i].height) << " "
                           ...
                           << std::to_string((detected_objects.objects[i].pose.position.y + detected_objects.objects[i].dimensions.z / 2) ) << " "
                           << std::to_string(detected_objects.objects[i].pose.position.z ) << " "
                           << std::to_string(detected_objects.objects[i].yaw) <<"\n";
                        //    << " " << detected_objects.objects[i].score <<"\n";
        }
        frame_count_++;
}
