                       ...
                       int in_g, int in_b, std::string in_label, bool in_estimate_pose)
{
  ...
  for (auto pit = in_cluster_indices.begin(); pit != in_cluster_indices.end(); ++pit)
  {
    ...
    p.intensity = in_origin_cloud_ptr->points[*pit].intensity;
    // p.r = in_r;
    // p.g = in_g;
    // p.b = in_b;
  ...
  if (current_cluster->points.size() > 3)
  {
    ...
    pcl::copyPointCloud<pcl::PointXYZI, pcl::PointXYZI>(*current_cluster, *current_cluster_mono);
                                              ...
                                              const double& in_fpfh_search_radius)
{
  ...
  pcl::search::KdTree<pcl::PointXYZI>::Ptr norm_tree(new pcl::search::KdTree<pcl::PointXYZI>);
