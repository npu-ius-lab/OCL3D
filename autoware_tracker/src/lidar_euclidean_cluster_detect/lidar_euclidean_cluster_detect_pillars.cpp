...
std::vector<std::vector<geometry_msgs::Point> > _way_areaextrinsic_calibration_points;
...
sensor_msgs::PointCloud2 extractPointCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_cloud_ptr,
                                           const autoware_tracker::DetectedObject detected_object)
{
    
    const geometry_msgs::Point& center = detected_object.pose.position;
    const geometry_msgs::Quaternion& orientation = detected_object.pose.orientation;
    float box_length = detected_object.dimensions.x;
    float box_width = detected_object.dimensions.y;
    float box_height = detected_object.dimensions.z;


    float min_x = center.x - (box_length / 2);
    float max_x = center.x + (box_length / 2);
    float min_y = center.y - (box_width / 2);
    float max_y = center.y + (box_width / 2);
    float min_z = center.z - (box_height / 2);
    float max_z = center.z + (box_height / 2);


    pcl::PointIndices::Ptr indices(new pcl::PointIndices);
    for (int i = 0; i < in_cloud_ptr->points.size(); ++i) {
        const pcl::PointXYZI& point = in_cloud_ptr->points[i];
    ...
    pcl::toROSMsg(*output_cloud_ptr, output_msg);
    output_msg.header = detected_object.header;

    return output_msg;
}

// IoU
double ComputeIoU(const std::vector<Eigen::Vector4d>& boxA, const std::vector<Eigen::Vector4d>& boxB) {
      
        double minAX = std::min_element(boxA.begin(), boxA.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.x() < p2.x(); })->x();
        double maxAX = std::max_element(boxA.begin(), boxA.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.x() < p2.x(); })->x();
        double minAY = std::min_element(boxA.begin(), boxA.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.y() < p2.y(); })->y();
        double maxAY = std::max_element(boxA.begin(), boxA.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.y() < p2.y(); })->y();

        double minBX = std::min_element(boxB.begin(), boxB.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.x() < p2.x(); })->x();
        double maxBX = std::max_element(boxB.begin(), boxB.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.x() < p2.x(); })->x();
        double minBY = std::min_element(boxB.begin(), boxB.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.y() < p2.y(); })->y();
        double maxBY = std::max_element(boxB.begin(), boxB.end(), [](const Eigen::Vector4d& p1, const Eigen::Vector4d& p2) { return p1.y() < p2.y(); })->y();
  
        double overlapArea = std::max(0.0, std::min(maxAX, maxBX) - std::max(minAX, minBX)) *
                                std::max(0.0, std::min(maxAY, maxBY) - std::max(minAY, minBY));
        ...
        double iou = overlapArea / (areaA + areaB - overlapArea);
        // double iou = overlapArea / (areaB);
        return iou;
}

std::vector<Eigen::Vector4d> getEightVertices(autoware_tracker::DetectedObject detectedObject){
        std::vector<Eigen::Vector4d> vertices_3dbox(8);
        float center_x = detectedObject.pose.position.x;
        float center_y = detectedObject.pose.position.y;
        float center_z = detectedObject.pose.position.z;

        Eigen::Quaterniond quaternion(detectedObject.pose.orientation.w, detectedObject.pose.orientation.x, detectedObject.pose.orientation.y, detectedObject.pose.orientation.z);
        ...
        transformation_matrix(0, 3) = center_x;
        transformation_matrix(1, 3) = center_y;
        transformation_matrix(2, 3) = center_z;
        Eigen::Vector4d dimensions(detectedObject.dimensions.x / 2, detectedObject.dimensions.y / 2, detectedObject.dimensions.z, 1);


        // std::vector<Eigen::Vector4d> corner_points(8);
        vertices_3dbox[0] = transformation_matrix * Eigen::Vector4d(dimensions.x(), dimensions.y(), dimensions.z(), 1);
        vertices_3dbox[1] = transformation_matrix * Eigen::Vector4d(dimensions.x(), -dimensions.y(), dimensions.z(), 1);
        vertices_3dbox[2] = transformation_matrix * Eigen::Vector4d(dimensions.x(), dimensions.y(), dimensions.z(), 1);
        vertices_3dbox[3] = transformation_matrix * Eigen::Vector4d(-dimensions.x(), dimensions.y(), dimensions.z(), 1);
        vertices_3dbox[4] = transformation_matrix * Eigen::Vector4d(dimensions.x(), -dimensions.y(), -dimensions.z(), 1);
        vertices_3dbox[5] = transformation_matrix * Eigen::Vector4d(dimensions.x(), dimensions.y(), -dimensions.z(), 1);
        vertices_3dbox[6] = transformation_matrix * Eigen::Vector4d(-dimensions.x(), dimensions.y(), -dimensions.z(), 1);
        vertices_3dbox[7] = transformation_matrix * Eigen::Vector4d(-dimensions.x(), -dimensions.y(), dimensions.z(), 1);
        return vertices_3dbox;
}

geometry_msgs::Vector3 estimate_size(autoware_tracker::DetectedObject in_object){
...
std::vector<Eigen::Vector4d> getEightVerticesCluster(autoware_tracker::DetectedObject in_object ) {
        std::vector<Eigen::Vector4d> vertices(8);
        geometry_msgs::Vector3 dimensions = estimate_size(in_object);
        double rot_z = quat2rot_z(in_object);
        rot_z = estimate_rot_z(in_object,rot_z);
        ...
        geometry_msgs::Pose pose = in_object.pose;
        ...
        for (int i = 0; i < 8; ++i) {
                vertices[i][0] = transformed_corners_3d(0, i);
                vertices[i][1] = transformed_corners_3d(1, i);
                vertices[i][2] = transformed_corners_3d(2, i);
                vertices[i][3] = 1;
        }
...
autoware_tracker::DetectedObjectArray getSameClass(std::string label,autoware_tracker::DetectedObjectArray dets){
        autoware_tracker::DetectedObjectArray dets_out;
        for (int i = 0; i < dets.objects.size();i++){
                if (dets.objects[i].label == label){
                        dets_out.objects.push_back(dets.objects[i]);
                }
        }
        return dets_out;
}
...
double computedis(geometry_msgs::Pose pose1,geometry_msgs::Pose pose2){
        double dis = sqrt(pow(pose1.position.x - pose2.position.x,2) + pow(pose1.position.y - pose2.position.y,2));
        return dis;
}
...
std::vector<autoware_tracker::DetectedObjectArray>  fusionBox(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,autoware_tracker::DetectedObjectArray detected_objects,const jsk_recognition_msgs::BoundingBoxArrayConstPtr& pillars_detections_sub){
        ...
        std::vector<autoware_tracker::DetectedObjectArray> fusionDetectedObjectArrayTwo(2);



        autoware_tracker::DetectedObjectArray pillarResults;

        autoware_tracker::DetectedObjectArray toPredictObjects;
        autoware_tracker::DetectedObjectArray toTrackerObjects;
        toTrackerObjects.frame_out = detected_objects.frame_out;
        toPredictObjects.frame_out = detected_objects.frame_out;

        toTrackerObjects.header = detected_objects.header;
        toPredictObjects.header = detected_objects.header;
        
        std::vector<bool> pillarsMatched(pillars_detections_sub->boxes.size(),false);
        
        for (int i = 0;i < pillars_detections_sub->boxes.size();i++){

                autoware_tracker::DetectedObject detected_object;
                detected_object.header = detected_objects.header;
                detected_object.pose = pillars_detections_sub->boxes[i].pose;
                detected_object.flag = true;
                detected_object.score = pillars_detections_sub->boxes[i].value; 
                detected_object.dimensions = pillars_detections_sub->boxes[i].dimensions;
                detected_object.pointcloud = extractPointCloud(in_cloud_ptr,detected_object);
                detected_object.label = std::to_string(pillars_detections_sub->boxes[i].label);

                pillarResults.objects.push_back(detected_object);
        }
        
        // std::vector<bool> clusterMatched(detected_objects.objects.size(),false);
        double iou_thre = 0.1;
        for (int i = 0;i < detected_objects.objects.size();i++){
                detected_objects.objects[i].flag = false; 
                if (detected_objects.objects[i].label == "9")
                {
                        continue;
                }
                
                std::vector<Eigen::Vector4d> vertices_3dbox_cluster(8);
                vertices_3dbox_cluster = getEightVerticesCluster(detected_objects.objects[i]);

                double iou = 0.0;
                std::vector<float> iou_list;
                std::vector<int> j_list;
                
                for (int j = 0; j < pillarResults.objects.size();j++){
                       
                        if (pillarResults.objects[j].label != detected_objects.objects[i].label)
                        { 
                                continue;
                        }
                        std::vector<Eigen::Vector4d> vertices_3dbox_pillars(8);
                        vertices_3dbox_pillars = getEightVertices(pillarResults.objects[j]);
                        iou = ComputeIoU(vertices_3dbox_cluster, vertices_3dbox_pillars);
                ...
                double iou_max = 0.0; 
                int pillars_j = 0;  
                for (int n = 0;n < iou_list.size();n++ ){
                        if (iou_max < iou_list[n]){ 
                               iou_max =  iou_list[n];
                               pillars_j = j_list[n];
                        } 
                }
                
                if (iou_max > iou_thre && pillarResults.objects[pillars_j].score >= 0.5)
                {       
                        pillarsMatched[pillars_j] = true;
                        toTrackerObjects.objects.push_back(pillarResults.objects[pillars_j]);
                }else{
                        toTrackerObjects.objects.push_back(detected_objects.objects[i]);
                }

                
        }

        toPredictObjects = toTrackerObjects;
        // int matched_cn = 0;

        for (size_t i = 0; i < pillarsMatched.size(); i++)
        {
                bool addNotMatchedPillar = true;
                if (!pillarsMatched[i] && pillarResults.objects[i].score >= 0.5){
                
                        if (addNotMatchedPillar)
                        {
                                toPredictObjects.objects.push_back(pillarResults.objects[i]);
                        }
                }
        }
        // std::cout << "matched "<<matched_cn << " not matched " << pillarsMatched.size() - matched_cn << std::endl;
        fusionDetectedObjectArrayTwo[0] = toTrackerObjects;


        std::vector<size_t> indices_to_remove; 

        for (size_t i = 0; i < toPredictObjects.objects.size(); ++i) {
                for (size_t j = i + 1; j < toPredictObjects.objects.size(); ++j) {
                        double dis = computedis(toPredictObjects.objects[i].pose, toPredictObjects.objects[j].pose);
               
                        if (dis < 0.3 && (toPredictObjects.objects[i].flag || toPredictObjects.objects[j].flag)) {
                                if (toPredictObjects.objects[i].flag) {
        ...
        for (size_t idx : indices_to_remove) {
                toPredictObjects.objects.erase(toPredictObjects.objects.begin() + idx);
        }

        fusionDetectedObjectArrayTwo[1] = toPredictObjects;

        std::cout << " to predict " << toPredictObjects.objects.size() << std::endl;
        return fusionDetectedObjectArrayTwo;
}
...
void publishDetectedObjects(const autoware_tracker::CloudClusterArray &in_clusters,
                            const vision_msgs::Detection2DArrayConstPtr& in_image_detections,const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,
                            const jsk_recognition_msgs::BoundingBoxArrayConstPtr& pillars_detections_sub)
{       double start_time;
        double current_time;
        ...
        detected_objects.frame_out = cout_in;
        ...
        for(size_t i = 0; i < in_clusters.clusters.size(); i++) {
                ...
                // Eigen::Vector4d velo_position = calib.getTr_velo_cam().inverse() * rect_position;
                ...
                detected_object.frame_count = cout_in;
                ...
                for(size_t j = 0; j < in_image_detections->detections.size(); j++) {
                        if(in_image_detections->detections[j].results[0].score > 0.3) {
                                ...
                                if(inter_area > 0) {
                                        ...
                                        if(iou > iou_max) {
                                                iou_max = iou;
                                                if(iou_max > iou_threshold && in_clusters.clusters[i].centroid_point.point.x > 0) {
        ...
        ROS_INFO("2D-3D association: %lf s", current_time - start_time);
        
        std::vector<autoware_tracker::DetectedObjectArray> fusionDetectedObjectArrayTwo;
        autoware_tracker::DetectedObjectArray fusionDetectedObjectArray;
        fusionDetectedObjectArray.header  = in_clusters.header;
        
        fusionDetectedObjectArrayTwo = fusionBox(in_cloud_ptr,detected_objects,pillars_detections_sub);
        fusionDetectedObjectArray.frame_out = cout_in;

        std::cout << " frame out " << fusionDetectedObjectArray.frame_out << std::endl;
        

        _pub_detected_objects.publish(fusionDetectedObjectArrayTwo[0]);
        _pub_detected_objects_Predict.publish(fusionDetectedObjectArrayTwo[1]);
...
void publishCloudClusters(const ros::Publisher *in_publisher,const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr, const autoware_tracker::CloudClusterArray &in_clusters,
                          ...
                          const jsk_recognition_msgs::BoundingBoxArrayConstPtr& pillars_detections_sub)
{
        // std::cout << " the pointpillars has " << pillars_detections_sub->boxes.size() << " boxes " << std::endl;
...
{
        pcl::search::KdTree<pcl::PointXYZI>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZI>); //利用KD树进行最近邻搜索
        ...
        {
                cloud_2d->points[i].z = 0;//z坐标置0，只在xy平面聚类
        }
        ...
        ec.setClusterTolerance(in_max_cluster_distance);          //点之间距离阈值
        ec.setMinClusterSize(_cluster_size_min);
        ec.setMaxClusterSize(_cluster_size_max);
        ec.setSearchMethod(tree);
        ec.setInputCloud(cloud_2d);
        ec.extract(cluster_indices); //cluster_indices为欧式聚类索引
        // use indices on 3d cloud

        /////////////////////////////////
        //---	3. Color clustered points   给聚类点云上色
        /////////////////////////////////
        unsigned int k = 0;
...
int cout_in_callback = -1;
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud,
                       const vision_msgs::Detection2DArrayConstPtr& in_image_detections,
                       const jsk_recognition_msgs::BoundingBoxArrayConstPtr& pillars_detections_sub)
{       
        cout_in_callback += 1;
        std::cout << " from -1 in cout_in_callback in autoware " << cout_in_callback <<std::endl;
        
        std::cerr << in_sensor_cloud->header.stamp << "---" << in_image_detections->header.stamp << "---" <<pillars_detections_sub->header.stamp << std::endl;
        ...
        pcl::PointCloud<pcl::PointXYZI>::Ptr nofloor_cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);                                                         //初始化点云对象，且从上到下为初始点云处理的顺序
        ...
        autoware_tracker::Centroids centroids; //定义中心点
        autoware_tracker::CloudClusterArray cloud_clusters; //定义clusters对象

        pcl::fromROSMsg(*in_sensor_cloud, *current_sensor_cloud_ptr); //将ros点云消息转化为pcl点云对象
        ...
        // height clip
        clipCloud(downsampled_cloud_ptr, clipped_cloud_ptr, _clip_min_height, _clip_max_height);  //过滤z轴方向点云  _clip_min_height, _clip_max_height

        // left and right points filter
        if (_keep_lanes)
                keepLanePoints(clipped_cloud_ptr, inlanes_cloud_ptr, _keep_lane_left_distance, _keep_lane_right_distance);//过滤y轴方向 （车道）点云 
        else
        ...
        if (_remove_ground)
        {
                removeFloor(inlanes_cloud_ptr, nofloor_cloud_ptr, onlyfloor_cloud_ptr); //过滤地面点云
        ...
        // normal filter
        if (_use_diffnormals)
                differenceNormalsSegmentation(nofloor_cloud_ptr, diffnormals_cloud_ptr);//法线差异分割，最终用于聚类的点云对象为diffnormals_cloud_ptr
        else
                diffnormals_cloud_ptr = nofloor_cloud_ptr;

        // main cluster
        start_time = ros::WallTime::now().toSec();
        segmentByDistance(diffnormals_cloud_ptr, colored_clustered_cloud_ptr, centroids, cloud_clusters);//基于距离分段聚类
        publishColorCloud(&_pub_cluster_cloud, colored_clustered_cloud_ptr);//发布点云colored_clustered_cloud_ptr
        current_time = ros::WallTime::now().toSec();
        ROS_INFO("cluster: %lf s", current_time - start_time);

        centroids.header = _velodyne_header;
        publishCentroids(&_centroid_pub, centroids, _output_frame, _velodyne_header);//发布质心坐标

        cloud_clusters.header = _velodyne_header;
        publishCloudClusters(&_pub_clusters_message,diffnormals_cloud_ptr ,cloud_clusters, _output_frame, _velodyne_header, in_image_detections,pillars_detections_sub); //发布聚类
}
...
int main(int argc, char **argv)
{
        ...
        message_filters::Subscriber<jsk_recognition_msgs::BoundingBoxArray> pillars_detections_sub(nh, "/detected_objects_vis_box", 10);


        typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::PointCloud2, vision_msgs::Detection2DArray ,jsk_recognition_msgs::BoundingBoxArray> MySyncPolicy;
        // ApproximateTime takes a queue size as its constructor argument, hence MySyncPolicy(10)
        message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), points_sub, image_detections_sub, pillars_detections_sub);
        sync.registerCallback(boost::bind(&velodyne_callback, _1, _2,_3));
        // yang21itsc
