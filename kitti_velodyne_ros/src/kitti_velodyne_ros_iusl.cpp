...
int main(int argc, char **argv) {
        ...
        private_nh.param<std::string>("velodyne_fog_dir", velodyne_fog_dir, "/home/lbh/deep_learning/SUSTechPOINTS-batch-edit/data_kitti/iusl_key_frame/noise/");
        private_nh.param<std::string>("visibility", visibility, "100");
        private_nh.param<bool>("isnoise", isnoise, false);
        ...
        if (isnoise)
        {
                std::cout << "velodyne_fog_dir " << velodyne_fog_dir << " visibility " << visibility << std::endl;
                velodyne_dir = velodyne_fog_dir + visibility + "/";
        }
        std::cout << "velodyne_dir " << velodyne_dir << std::endl;
        ...
        while(ros::ok() && i_file < n_file) {
                ...
                if(!velodyne_bin.good()) {
                        ...
                        for(int i = 0; velodyne_bin.good() && !velodyne_bin.eof(); i++) {
                                ...
                                {
                                        pcl::PointXYZ point2;
                                        velodyne_bin.read((char *) &point2.x, 2 * sizeof(float));
                                }
                        ...
                        count += 1;
                        std::cout<< "in kitti velodyne  ros iusl "<< count << std::endl;
                }
